...
public class LightAir {
	...
	public class GuiHandler implements IGuiHandler {
		...
		public Object getServerGuiElement(int ID, EntityPlayer player, World world, int x, int y, int z) {
			if (ID == 0) {
				...
				if (s != null && s.getItem() == Item.getItemFromBlock(CommonProxy.BLOCK_LIGHT_AIR))
	...
	public class Config {
		...
		public Config(Configuration conf) {
			...
			maxChunkRadius = configuration.getInt(key, category, defaultMaxChunkRadius, minMaxChunkRadius,
					maxMaxChunkRadius, comment);
		}
