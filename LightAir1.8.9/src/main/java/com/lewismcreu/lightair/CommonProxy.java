...
public class CommonProxy
{
    ...
    public static final Item glowstoneBit = new Item().setUnlocalizedName("glowstone_bit").setRegistryName(
            "glowstone_bit").setCreativeTab(CreativeTabs.tabMaterials);

	public void preInit()
	{
		GameRegistry.registerBlock(blockAirLight, ItemBlockLightAir.class);
	...
	public void init()
	{
		...
		GameRegistry.addShapelessRecipe(new ItemStack(coalDust, 4), new ItemStack(Items.coal));
		GameRegistry.addShapelessRecipe(new ItemStack(coalBit, 4), new ItemStack(coalDust));
		...
		GameRegistry.addRecipe(new IRecipe()
		{
			...
			public boolean matches(InventoryCrafting inv, World worldIn)
			{
				...
				for (int i = 0; i < inv.getSizeInventory(); i++)
					if (inv.getStackInSlot(i) != null) input.add(inv.getStackInSlot(i));
			...
			{
				return in != null ? in.getItem() == Item.getItemFromBlock(blockAirLight) || OreDictionary.itemMatches(
						new ItemStack(Items.glowstone_dust), in, false) || OreDictionary.itemMatches(new ItemStack(
								coalDust), in, false) || OreDictionary.itemMatches(new ItemStack(coalBit), in, false)
						|| OreDictionary.itemMatches(new ItemStack(glowstoneBit), in, false) : false;
			}
			...
			public ItemStack getCraftingResult(InventoryCrafting inv)
			{
				...
				ItemStack out = new ItemStack(Item.getItemFromBlock(blockAirLight));
				int meta = 0;
				int lighter = 0;
				for (int i = 0; i < inv.getSizeInventory(); i++)
				{
					...
					if (in != null)
					{
						if (in.getItem() == Item.getItemFromBlock(blockAirLight)) meta = in.getItemDamage();
						else if (OreDictionary.itemMatches(new ItemStack(Items.glowstone_dust), in, false)) lighter +=
								4;
						else if (OreDictionary.itemMatches(new ItemStack(coalDust), in, false)) lighter -= 4;
						else if (OreDictionary.itemMatches(new ItemStack(coalBit), in, false)) lighter--;
						else if (OreDictionary.itemMatches(new ItemStack(glowstoneBit), in, false)) lighter++;
					}
				}
				meta += lighter;
